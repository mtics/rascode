...
def test_show_main_text_returns_str():
    ...
    from rascode.mcp.server import show_main_text

    result = show_main_text([])
...
def test_show_left_oled_returns_str():
    ...
    from rascode.mcp.server import show_left_oled

    result = show_left_oled([])
    assert isinstance(result, str)
    assert len(result) > 0
...
def test_show_right_oled_returns_str():
    ...
    from rascode.mcp.server import show_right_oled

    result = show_right_oled([])
    assert isinstance(result, str)
    assert len(result) > 0
...
def test_clear_screen_returns_str():
    """clear_screen 返回字符串。"""
    from rascode.mcp.server import clear_screen

    result = clear_screen("all")
    assert isinstance(result, str)
    assert len(result) > 0
...
def test_restore_dashboard_returns_str():
    ...
    from rascode.mcp.server import restore_dashboard

    result = restore_dashboard()
    assert isinstance(result, str)
    assert len(result) > 0
